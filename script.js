...
document.addEventListener('DOMContentLoaded', function() {
    ...
    loadButton.addEventListener('click', async function() {
        ...
        loadButton.textContent = 'Loading Restaurant Data...';
        //update status
        ...
        try {
            ...
            //get restaurant data from GeoJSON
            ...
            statusDisplay.querySelector('.status-message').textContent = `Successfully loaded ${restaurants.length} restaurants`;
            //enable interface
            showDataSummary();
            showInitialView();
            ...
            statusDisplay.classList.remove('loading', 'success');
            statusDisplay.classList.add('error');
            statusDisplay.querySelector('.status-message').textContent = 'Failed to load data.';
            //reset button
    ...
    function showCardView() {
        ...
        // YOUR CODE HERE:
        //create cards for restaurant discovery
        restaurants.forEach(function(restaurant) {
            ...
            card.innerHTML = `
                ...
                <p>Address: ${props.address_line_1}</p>
                <p>Status: ${props.inspection_results}</p>
                `;
    ...
    function showTableView() {
        ...
        restaurants.slice(0,50).forEach(function(restaurant) {
            ...
            row.innerHTML = `
                ...
                <td>${props.inspection_results}</td>
                <td>${getComplianceIndicator(props.adequate_hand_washing)}</td>
                <td>${getComplianceIndicator(props.cooking_time_and_temperature)}</td>
                `;
    ...
    function showStatsView() {
        ...
        const compliantPct = total ? ((compliantCount / total) * 100).toFixed(1) : 0;
        const nonCompliantPct = total ? ((nonCompliantCount / total) * 100).toFixed(1) : 0;

        //group by city and calculate compliance counts per city
        const cityStats = {};
        restaurants.forEach(r => {
            ...
            else if (status === 'non-compliant') cityStats[city].nonCompliant++;
        });

        //build HTML to display stats
        const statsDiv = document.querySelector('#stats-view');
        statsDiv.innerHTML = `
            <h2>Restaurant Inspection Summary</h2>
            <p>Total restaurants: ${total}</p>
            <p>Compliant: ${compliantCount} (${compliantPct}%)</p>
            <p>Non-compliant: ${nonCompliantCount} (${nonCompliantPct}%)</p>
            <p>Unknown: ${otherCount}</p>
            <h3>Top Cities by Number of Restaurants</h3>
            <ul>
                ${Object.entries(cityStats)
                    ...
                    .map(([city, stats]) => `
                    <li>
                        ${city}: ${stats.total} restaurants - 
                        Compliant: ${stats.compliant}, Non-Compliant: ${stats.nonCompliant}
                    </li>
                `)
                .join('')}
            <ul>
    ...
    function showDataSummary() {
        ...
        //get unique cities
        const uniqueCities = new Set(restaurants.map(r => r.properties.city)).size;

        //update data summary UI
        dataSummary.innerHTML = `
            <p>Total Restaurants: ${total}</p>
            <p>Compliance Rate: ${compliantPct}%</p>
            <p>Unique Cities: ${uniqueCities}</p>
        `
